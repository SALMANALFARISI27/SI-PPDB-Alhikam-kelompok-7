...
class Download extends BaseController
{
	...
	public function kategori_download($id_kategori_download)
	{
		...
		$data = [
			'title' => $kategori_download->nama_kategori_download . ' (' . count($download) . ')',
			'download' => $download,
			'content' => 'admin/download/index'
		];
		echo view('admin/layout/wrapper', $data);
	...
	public function author($id_admin)
	{
		...
		$download = $m_download->author_all($id_admin);
		$total = $m_download->total_author($id_admin);
	...
	public function proses()
	{
		...
		if ($submit == 'Update') {
			for ($i = 0; $i < sizeof($id_download ?? []); $i++) {
				$data = array(
					'id_download' => $id_download[$i],
					'id_admin' => $this->session->get('id_admin'),
					'id_kategori_download' => $this->request->getVar('id_kategori_download')
				);
				$m_download->edit($data);
			}
			return redirect()->to($pengalihan)->with('sukses', 'Download berhasil diupdate kategorinya');
