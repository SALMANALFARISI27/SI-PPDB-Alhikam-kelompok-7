	...
	public function index()
	{
		...
		if (
			...
			if (!empty($_FILES['gambar']['name'])) {
				...
				$avatar->move(FCPATH . 'assets/upload/image/', $nama_kategori_ekstrakurikulerbaru);
				// Create thumb
				$image = \Config\Services::image()
					->withFile(FCPATH . 'assets/upload/image/' . $nama_kategori_ekstrakurikulerbaru)
					->fit(100, 100, 'center')
					->save(FCPATH . 'assets/upload/image/thumbs/' . $nama_kategori_ekstrakurikulerbaru);
				// masuk database
			...
			$data = [
				'title' => 'Data Kategori Ekstrakurikuler: ' . $total->total,
	...
	public function edit($id_kategori_ekstrakurikuler)
	{
		...
		if (
			...
			if (!empty($_FILES['gambar']['name'])) {
				...
				$slug = strtolower(url_title($this->request->getVar('nama_kategori_ekstrakurikuler')));
				$data = [
					'id_kategori_ekstrakurikuler' => $id_kategori_ekstrakurikuler,
					'id_admin' => $this->session->get('id_admin'),
					'slug_kategori_ekstrakurikuler' => $slug,
					'nama_kategori_ekstrakurikuler' => $this->request->getPost('nama_kategori_ekstrakurikuler'),
					'keterangan' => $this->request->getPost('keterangan'),
			...
			$data = [
				'title' => 'Edit Kategori Ekstrakurikuler: ' . $kategori_ekstrakurikuler->nama_kategori_ekstrakurikuler,
				'kategori_ekstrakurikuler' => $kategori_ekstrakurikuler,
