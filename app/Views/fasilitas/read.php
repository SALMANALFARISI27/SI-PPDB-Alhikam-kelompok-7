        ...
        <div class="row grid-view gx-md-8 gx-xl-10 gy-8 gy-lg-0">
         ...
         <div class="col-md-4">
          <div class="card">
            <div class="card-body">
              <?php
                $imgPublic = FCPATH . 'assets/upload/image/' . $fasilitas->gambar;
                $imgRoot   = ROOTPATH . 'assets/upload/image/' . $fasilitas->gambar;
                if(file_exists($imgPublic)) {
                    $imgUrl = base_url('assets/upload/image/'.$fasilitas->gambar);
                } elseif(file_exists($imgRoot)) {
                    if(!is_dir(FCPATH . 'assets/upload/image/')) { mkdir(FCPATH . 'assets/upload/image/', 0777, true); }
              ...
              <div class="card">
                <figure class="card-img-top">
                  <a href="<?php echo base_url('fasilitas/read/'.$fasilitas->slug_fasilitas) ?>">
                    <?php
                      $imgPublicSide = FCPATH . 'assets/upload/image/' . $fasilitas->gambar;
                      $imgRootSide   = ROOTPATH . 'assets/upload/image/' . $fasilitas->gambar;
                      if(file_exists($imgPublicSide)) {
                          $imgUrlSide = base_url('assets/upload/image/'.$fasilitas->gambar);
                      } elseif(file_exists($imgRootSide)) {
                          if(!is_dir(FCPATH . 'assets/upload/image/')) { mkdir(FCPATH . 'assets/upload/image/', 0777, true); }
                          copy($imgRootSide, $imgPublicSide);
                          $imgUrlSide = base_url('assets/upload/image/'.$fasilitas->gambar);
                      } else {
                          $imgUrlSide = base_url('assets/images/placeholder.jpg');
                      }
                    ?>
                    <img class="img-fluid" src="<?php echo $imgUrlSide ?>" srcset="<?php echo $imgUrlSide ?> 2x" alt="<?php echo $fasilitas->judul_fasilitas ?>" />
                  </a>
                </figure>
