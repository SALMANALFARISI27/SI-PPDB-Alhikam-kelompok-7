                  ...
                  <figure class="card-img-top overlay overlay-1 hover-scale">
                   ...
                   <?php 
                    if ($staff->gambar == "") { 
                        ...
                        // Tentukan lokasi file gambar di folder dalam dan luar
                        $img_dipublic = FCPATH . 'assets/upload/staff/' . $staff->gambar;
                        $img_diluar   = FCPATH . '../assets/upload/staff/' . $staff->gambar;

                        // Jika file belum ada di folder public tapi ada di luar, salin ke public
                        if (!file_exists($img_dipublic) && file_exists($img_diluar)) {
                            @copy($img_diluar, $img_dipublic);
                        }
                    ?>
                        <img src="<?php echo base_url('assets/upload/staff/' . $staff->gambar) ?>" 
                            alt="<?php echo $title ?>" 
                            class="img-thumbnail">
                    <?php 
                    } 
