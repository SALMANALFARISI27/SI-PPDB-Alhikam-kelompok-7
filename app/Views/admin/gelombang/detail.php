<p class="text-right">
	...
	<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Semua/Semua') ?>"
		class="btn btn-warning btn-xs mb-1">
		<i class="fa fa-users"></i> Semua Jenjang
		(<?php echo $this->website->angka($m_calon_peserta_didik->total_gelombang_status_calon_peserta_didik($id_gelombang, 'Semua', 'Semua')->total); ?>)
	</a>
	<?php if ($id_jenjang_pendidikan != 'Semua') { ?>
		<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Semua/' . $id_jenjang_pendidikan) ?>"
			class="btn btn-info btn-xs mb-1">
			<i class="fa fa-user-check"></i> <?php echo $judul_jenjang_pendidikan ?>
	...
	<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Menunggu/' . $id_jenjang_pendidikan) ?>"
		class="btn btn-dark btn-xs mb-1">
		<i class="fa fa-tasks"></i> Menunggu
	...
	<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Diterima-Tahap-1/' . $id_jenjang_pendidikan) ?>"
		class="btn btn-success btn-xs mb-1">
		<i class="fa fa-check-circle"></i> Diterima Tahap 1
		(<?php echo $this->website->angka($m_calon_peserta_didik->total_gelombang_status_calon_peserta_didik($id_gelombang, 'Diterima-Tahap-1', $id_jenjang_pendidikan)->total); ?>)
	</a>
	<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Tidak-Diterima/' . $id_jenjang_pendidikan) ?>"
		class="btn btn-danger btn-xs mb-1">
		<i class="fa fa-times-circle"></i> Tidak Diterima
	...
	<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Diperiksa/' . $id_jenjang_pendidikan) ?>"
		class="btn btn-secondary btn-xs mb-1">
		<i class="fa fa-edit"></i> Diperiksa
	...
	<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Lulus/' . $id_jenjang_pendidikan) ?>"
		class="btn btn-primary btn-xs mb-1">
		<i class="fa fa-graduation-cap"></i> Lulus
		(<?php echo $this->website->angka($m_calon_peserta_didik->total_gelombang_status_calon_peserta_didik($id_gelombang, 'Lulus', $id_jenjang_pendidikan)->total); ?>)
	</a>

	<a href="<?php echo base_url('admin/gelombang/export/' . $gelombang->id_gelombang . '/' . $status_pendaftaran . '/' . $id_jenjang_pendidikan) ?>"
		class="btn btn-success btn-xs mb-1" target="_blank"><i class="fa fa-file-excel"></i> Ekspor Calon Peserta
		Didik</a>
	<a href="<?php echo base_url('admin/gelombang/unduh_data/' . $gelombang->id_gelombang . '/' . $status_pendaftaran . '/' . $id_jenjang_pendidikan) ?>"
				...
				<?php foreach ($akumulasi as $akumulasi) { ?>
					<tr>
						...
						<td>
							...
							<?php } elseif ($akumulasi->status_pendaftaran == 'Diterima-Tahap-1') { ?>
								<span class="badge badge-success"><i class="fa fa-check-circle"></i>&nbsp;Diterima Tahap
									1</span>
							...
							<?php } elseif ($akumulasi->status_pendaftaran == 'Lulus') { ?>
						...
						<td><?php echo $this->website->angka($akumulasi->jumlah_calon_peserta_didik) ?></td>
						<td>
							<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/' . $akumulasi->status_pendaftaran . '/' . $akumulasi->id_jenjang_pendidikan) ?>"
								class="btn btn-secondary btn-xs mb-1">
								<i class="fa fa-user-check"></i> Lihat
							</a>
							<a href="<?php echo base_url('admin/gelombang/export/' . $gelombang->id_gelombang . '/' . $akumulasi->status_pendaftaran . '/' . $akumulasi->id_jenjang_pendidikan) ?>"
								class="btn btn-secondary btn-xs mb-1" target="_blank">
								<i class="fa fa-file-excel"></i> Ekspor
							</a>
							<a href="<?php echo base_url('admin/gelombang/unduh_data/' . $gelombang->id_gelombang . '/' . $akumulasi->status_pendaftaran . '/' . $akumulasi->id_jenjang_pendidikan) ?>"
								class="btn btn-secondary btn-xs mb-1" target="_blank">
								<i class="fa fa-file-pdf"></i> Unduh
							</a>
							<a href="<?php echo base_url('admin/gelombang/unduh_pengumuman/' . $gelombang->id_gelombang . '/' . $akumulasi->status_pendaftaran . '/' . $akumulasi->id_jenjang_pendidikan) ?>"
								class="btn btn-danger btn-xs mb-1" target="_blank">
		...
		<div class="col-md-12">
			<div class="input-group">
				<select name="status_pendaftaran" class="form-control" required>
					...
					<option value="Menunggu" <?php echo ($status_pendaftaran == 'Menunggu') ? 'selected' : ''; ?>>Menunggu
					</option>
					<option value="Diterima-Tahap-1" <?php echo ($status_pendaftaran == 'Diterima-Tahap-1') ? 'selected' : ''; ?>>Diterima Tahap 1</option>
					<option value="Tidak-Diterima" <?php echo ($status_pendaftaran == 'Tidak-Diterima') ? 'selected' : ''; ?>>Tidak Diterima</option>
					<option value="Diperiksa" <?php echo ($status_pendaftaran == 'Diperiksa') ? 'selected' : ''; ?>>
						Diperiksa</option>
					<option value="Lulus" <?php echo ($status_pendaftaran == 'Lulus') ? 'selected' : ''; ?>>Lulus</option>
				</select>
				<span class="input-group-btn">
					<button type="submit" class="btn btn-info" name="submit" value="update"><i class="fa fa-save"></i>
						Update Status PPDB</button>
				</span>
				<a href="<?php echo base_url('admin/gelombang/detail/' . $gelombang->id_gelombang . '/Semua/Semua') ?>"
					class="btn btn-warning">
					<i class="fa fa-users"></i> Lihat Semua Jenjang
					(<?php echo $this->website->angka($m_calon_peserta_didik->total_gelombang_status_calon_peserta_didik($id_gelombang, 'Semua', 'Semua')->total); ?>)
				</a>
			</div>
		</div>
					...
					<tr>
						<td class="text-center">
							<div class="icheck-primary">
								<input type="checkbox" name="id_calon_peserta_didik[]"
									value="<?php echo $calon_peserta_didik->id_calon_peserta_didik ?>"
									id="check<?php echo $i ?>">
						...
						<td><strong><?php echo $calon_peserta_didik->nama_calon_peserta_didik ?></strong>
							<small>
								...
								<br><span class="text-secondary">TTL:</span>
								<?php echo $calon_peserta_didik->tempat_lahir ?>,
								<?php echo $this->website->tanggal_id($calon_peserta_didik->tanggal_lahir) ?>
								<br><span class="text-secondary">Kelamin:</span> <?php if ($calon_peserta_didik->jenis_kelamin == 'L') {
									echo 'Laki-laki';
								} else {
									echo 'Perempuan';
								} ?>
