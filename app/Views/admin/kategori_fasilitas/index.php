...
<table class="table table-bordered table-sm" id="example3">
	...
	<tbody>
		<?php
		$no = 1;
		foreach ($kategori_fasilitas as $kategori_fasilitas) {
			$fasilitas = $m_kategori_fasilitas->fasilitas($kategori_fasilitas->id_kategori_fasilitas);
			?>
			<tr>
				<td class="text-center"><?php echo $no ?></td>
				<td class="text-center">
					<?php if ($kategori_fasilitas->gambar == "") {
						echo '-';
					} else {
						$img_dipublic = FCPATH . 'assets/upload/image/thumbs/' . $kategori_fasilitas->gambar;
						$img_diluar = FCPATH . 'thumbs/' . $kategori_fasilitas->gambar;
						if (!file_exists($img_dipublic) && file_exists($img_diluar)) {
							@copy($img_diluar, $img_dipublic);
						}
						?>
						<img src="<?php echo base_url('assets/upload/image/thumbs/' . $kategori_fasilitas->gambar) ?>"
				...
				<td class="text-center"><?php if ($fasilitas) {
					echo $fasilitas->total;
				} else {
					echo 0;
				} ?> Fasilitas</td>
				<td class="text-center"><?php echo $kategori_fasilitas->urutan ?></td>
				<td class="text-center">
					<a href="<?php echo base_url('admin/kategori_fasilitas/edit/' . $kategori_fasilitas->id_kategori_fasilitas) ?>"
						class="btn btn-secondary btn-sm"><i class="fa fa-edit"></i></a>
					<a href="<?php echo base_url('admin/kategori_fasilitas/delete/' . $kategori_fasilitas->id_kategori_fasilitas) ?>"
