		...
		foreach ($kategori_download as $kategori_download) {
			...
			<tr>
				...
				<td class="text-center">
					<?php if ($kategori_download->gambar == "") {
						echo '-';
					} else {
						$img_dipublic = FCPATH . 'assets/upload/image/' . $kategori_download->gambar;
						$img_diluar = FCPATH . '' . $kategori_download->gambar;
						if (!file_exists($img_dipublic) && file_exists($img_diluar)) {
							@copy($img_diluar, $img_dipublic);
						}
						?>
						<img src="<?php echo base_url('assets/upload/image/' . $kategori_download->gambar) ?>"
				...
				<td class="text-center"><?php if ($download) {
					echo $download->total;
				} else {
					echo 0;
				} ?> Download</td>
				...
				<td class="text-center">
					...
					<a href="<?php echo base_url('admin/kategori_download/delete/' . $kategori_download->id_kategori_download) ?>"
